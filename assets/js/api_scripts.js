...
//        });
    ...
    $(document).on('click', '._curl', function() {
        $(this).ajaxSubmit(
                {
            url: $(this).attr('url'),
            cache: false,
            type: 'post',
            data: {body : $('textarea#_curl').val(),
                    url :   $('#form-view form').attr('action'),
                    },
            dataType: 'json',
            beforeSend: function(xhr) {
                setHeader(xhr)
...
function setHeader(xhr) {
    var  _authentication_key = $("[name='Authentication-Key']").val();

    if (typeof _authentication_key !== 'undefined')
        xhr.setRequestHeader('Authentication-Key', _authentication_key);
