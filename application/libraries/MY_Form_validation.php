...
class MY_Form_validation extends CI_Form_validation
{
    ...
    function unique($value, $params)
    {
        ...
        if (isset($fields[1])) {
            ...
            if (isset($where_value)) {
                ...
                $this->CI->db->where($where_table . ' !=', $this->CI->input->post($where_field));
            }
        }
    ...
    /**
     * Match CSV count of field to another
     *
     * @access    public
     * @param    string
     * @param    field
     * @return    bool
     */
    public function match_csv_count($str, $field)
    {
        ...
        if (count(explode(',', $str)) !== count(explode(',', $field))) {
            $this->CI->form_validation->set_message('match_csv_count', 'The Count of Comma separated Values should match.');
            return FALSE;
        }
        return TRUE;
    }
    ...
    {
        if (empty($str) && json_decode($str) == FALSE) {
            $this->CI->form_validation->set_message('valid_json', '%s must be valid JSON data.');
            return FALSE;
        }
        return TRUE;
    }
    ...
    function isUnique($value, $params)
    {
        ...
        $this->CI->form_validation->set_message('isUnique', 'The value in &quot;%s&quot; is already being used.');

        $fields = explode(",", $params);
        list($table, $field) = explode(".", $fields[0], 2);
        $this->CI->db->select($field)->from($table)->where($field, $value)->limit(1);

        if (isset($fields[1])) {
            list($where_table, $where_value) = explode(".", $fields[1], 2);
            if (isset($where_value)) {
                $this->CI->db->where($where_table . ' !=', $where_value);
            }
        }
        $query = $this->CI->db->get();
    ...
    function validate_date($date, $format = "Y-m-d H:i:s")
    {
        ...
        if ($d && strtotime($d) == strtotime($date)) {
            return TRUE;
        }
        $this->CI->form_validation->set_message('validate_date', 'The value in &quot;%s&quot; is invalid.');
        return FALSE;
    }

    /**
     * Check if valid Latitude,Longitude data
     *
     * @access    public
     * @param    string
     * @return    bool
     */
    public function valid_lat_lng($str)
    {
        $regex = '/^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?),\s*[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)$/';
        if (empty($str) || !(preg_match($regex, $str))) {
            $this->CI->form_validation->set_message('valid_lat_lng', '%s must be valid Latitude,Longitude data.');
            return FALSE;
        }
        return TRUE;
    }
