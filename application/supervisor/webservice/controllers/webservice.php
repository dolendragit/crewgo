...
class Webservice extends Web_Service_Controller
{
    ...
    public function read_message(){
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('message_id', 'Job ID', 'trim|required|is_natural_no_zero');
            if ($this->form_validation->run() == FALSE) {
                return $this->response('0000', array('msg' => validation_errors()));
            }
            $msg_id = $this->input->post('message_id', true);
            $read = $this->supervisor_model->read_message($msg_id, $user->id);

            if($read){
                return $this->response('0001');
            }
            return $this->response('0000', array('msg'=>'Unable to read'));

        } else {
            $this->load->view('job/message_details', array('title' => 'Read Message'));
        }
    }
    ...
    public function view_reply()
    {
        if ($this->input->post()) {
            ...
            $response['message'] = $this->supervisor_model->get_message_reply($message_id, $user->id);
            return $this->response('0001', $response);
        } else {
    ...
    public function delete_message($msg_id=0)
    {
        ...
        //     return $this->response('0001');
        // } else
        return $this->response('0000', array('msg'=>'This feature is removed.'));
    }
