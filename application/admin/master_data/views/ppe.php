	...
	<span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span>		  
                    ...
                    <form id="frm_ppe" name="frm_ppe" data-parsley-validate class="form-horizontal form-label-left" onsubmit=" return validateForm()" method="post">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input type="text" id="name" name="name" value="<?php echo set_value('name',@$ppe->name);?>"   class="form-control col-md-7 col-xs-12" maxlength="300">
                        </div>
                      </div>
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="last-name">Status<span class="required">*</span>
                        </label>
                        <div class="col-md-2 col-sm-2 col-xs-2" id="master_data_status_<?php echo $ppe->id;?>" style="margin-top:5px;">
                            <?php if($page_action=='edit'){ ?>
                             
                              <?php echo  getStatusData($ppe->id,$ppe->status,'ppe');?>
                              
                              <?php }else{ ?>
                            <select name="status" id="status" class="form-control col-md-4 col-xs-4">
                                
                                <option value="1" <?php echo  set_select('status', 1,@$ppe->status == '1'?true:true); ?>>Show</option>
                                 <option value="0" <?php echo  set_select('status', 0,@$ppe->status == '0'?true:false); ?>>Hide</option>
                            </select>
                              <?php } ?>
                      ...
                      <div class="form-group">
                        ...
                        <input type="hidden" name="hidden_id" id="hidden_id" value="<?php echo @$ppe->id;?>">
			...
			<div class="row">
                ...
                {
                    $s=1;
                    foreach($ppes as $ppe_data)
                    {
                ?>
                    <tr>
                      <td><?php echo $s;?></td>
                      <td><?php echo $ppe_data->name;?></td>
                       
                      <td id="master_data_status_<?php echo $ppe_data->id;?>">
                          <?php
                          echo  getStatusData($ppe_data->id,$ppe_data->status,'ppe');
                         ?>
                      </td>
                      <td>
                        <a href="<?php echo base_url('admin/master_data/ppe/edit').'/'.$ppe_data->id;?>" title="Edit"><span class="glyphicon glyphicon-edit"></span></a>&nbsp;&nbsp;
                        <a href="<?php echo base_url('admin/master_data/ppe/remove').'/'.$ppe_data->id;?>" onclick="return confirm('Do you really want to delete this data?')" title="Delete"><span class="glyphicon glyphicon-remove"></span></a>
                      </td>
                    </tr>
                <?php 
                        $s++;
