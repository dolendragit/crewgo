...
class Customerweb_model extends MY_Model 
{
...
/*    public function get_levels($user_id=""){
    ...
    public function get_user_by_email($email=''){
        if(!empty($email)){
            $result = $this->db->select('u.*, g.group_id, g.user_id')->from('tbl_user as u')
            ->join('tbl_user_group as g', 'g.user_id = u.id')
            ->where('g.group_id ',CUSTOMER)->where('u.email', $email)->where('u.forgotten_password_code IS NOT NULL ')->get()->row();
