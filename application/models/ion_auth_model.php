...
class Ion_auth_model extends CI_Model
{
    ...
    public function hash_password_db_send($id, $password, $use_sha1_override = FALSE)
    {
        if (empty($id) || empty($password)) {
            return FALSE;
        }
        ...
        // bcrypt
        if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt') {
            $hash = crypt($password, $hash_password_db->password);
            return $hash; //==$hash_password_db->password
        }
     ...
     * @author Mathew
     **/
    public function login($identity, $password, $remember = FALSE, $user_group = "")
    {
        ...
        if ($user_group == "") {
            ...
            $this->db->select('tbl_user.' . $this->identity_column . ', tbl_user.email, tbl_user.id, password, tbl_user.active, last_login');
            $this->db->from('tbl_user');
            $this->db->join('tbl_user_group', 'tbl_user.id = tbl_user_group.user_id', 'inner');
            $this->db->where('tbl_user.email', $identity);
//			$this->db->where('tbl_user.email_verified', '1');
            $this->db->where('tbl_user_group.group_id', $user_group);
            $query = $this->db->get();
        }
